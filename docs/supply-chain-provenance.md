...
This project now treats generated demo reports as build subjects. The CI workflow uploads them as review artifacts and signs their provenance with GitHub artifact attestations.
...
The Sigstore policy-controller manifest opts the `mlops` namespace into verification and starts with `no-match-policy: warn`. Production rollout should move to deny mode only after every image has digest-pinned signatures and provenance attestations.
...
gh attestation verify .local/reports --repo kevinmeix1/advanced-kubernetes-mlops-platform
cosign verify-attestation --type slsaprovenance ghcr.io/kevinmeix1/advanced-kubernetes-mlops-platform@sha256:<digest>
```

## Production Notes

In a real cluster, the same pattern should be applied to container images, model artifacts, and SBOMs. The next hardening step would add image builds, SBOM generation, vulnerability scan attestations, digest-pinned Kubernetes manifests, and scheduled OpenSSF Scorecard-style repository posture checks.
