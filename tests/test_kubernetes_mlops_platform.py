...
from kube_mlops_platform.demo_cockpit import build_judge_demo_cockpit
...
from kube_mlops_platform.event_driven_assets import build_event_driven_assets_plan
from kube_mlops_platform.flavor_fungibility import build_flavor_fungibility_plan
...
from kube_mlops_platform.hpa_scale_to_zero import build_hpa_scale_to_zero_plan
...
from kube_mlops_platform.multi_team_readiness import build_multi_team_readiness_plan
...
from kube_mlops_platform.suspended_job_resources import build_suspended_job_resource_plan
...
class KubernetesMLOpsPlatformTest(unittest.TestCase):
    ...
    def test_resource_health_status_plan_and_kubernetes_assets_exist(self) -> None:
        repo = Path(__file__).resolve().parents[1]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_dra_resource_health_runbook")
    ...
    def test_admin_access_diagnostics_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_dra_admin_access_diagnostics")
            self.assertEqual(report["feature"]["feature_gate"], "DRAAdminAccess")
            self.assertTrue(any(item["claim"] == "canary-gpu-admin-health" for item in report["diagnostics"]))
            self.assertTrue((root / "reports" / "admin_access_diagnostics_plan.json").exists())
        for expected in ["resource.kubernetes.io/admin-access", "adminAccess: true", "mlops-dra-admin", "DRAAdminAccessClaimRunningTooLong"]:
            self.assertIn(expected, manifest)
        for expected in ["DRA AdminAccess Diagnostics", "AdminAccess", "Least-privilege RBAC", "ResourceHealthStatus"]:
            self.assertIn(expected, docs)
    ...
    def test_inplace_resize_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_inplace_resize_release_controls")
        ...
        for expected in ["--subresource resize", "resizePolicy", "InPlaceOrRecreate", "PodResizePending", "InPlaceResizeInProgressTooLong"]:
            self.assertIn(expected, manifest)
        for expected in ["In-Place Pod Resize Controls", "pods/resize", "PodResizePending", "InPlaceOrRecreate"]:
            self.assertIn(expected, docs)
    ...
    def test_model_cache_plan_and_kserve_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_release_model_cache_gate")
            self.assertTrue(report["release_policy"]["promotion_requires_cache_evidence"])
            self.assertEqual(report["release_policy"]["missing_cache_action"], "freeze_candidate_promotion_and_keep_champion")
            self.assertEqual(report["cache_policy"]["namespace_scope"], "LocalModelNamespaceCache")
            self.assertEqual(report["status_gates"]["minimum_candidate_copies"], 2)
        ...
        for expected in ["LocalModelNamespaceCache", "LocalModelNodeGroup", "sourceModelUri", "modelSize", "nodeGroups", "oci://", "churn-risk-predictor-modelcar"]:
            self.assertIn(expected, manifest)
        for expected in ["Release Model Cache", "MLflow", "ModelDownloaded", "PVC"]:
            self.assertIn(expected, docs)
    ...
    def test_dag_bundle_versioning_plan_and_airflow_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_airflow3_git_dag_bundle_versioning")
            ...
            self.assertTrue(any(check["name"] == "scheduler_managed_backfill_policy" for check in report["checks"]))
    ...
    def test_asset_partitioning_plan_and_airflow_assets_exist(self) -> None:
        ...
        dag = (repo / "airflow" / "dags" / "partitioned_release_assets_dag.py").read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            report = build_asset_partitioning_plan(root)

            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_airflow_asset_partitioning_for_release_assets")
            self.assertIn("PartitionedAssetTimetable", report["features"]["timetables"])
            self.assertTrue(any(flow["partition_key"] == "model_version:hour" for flow in report["flows"]))
    ...
    def test_multi_team_readiness_plan_and_airflow_config_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "prepare_airflow_multi_team_release_isolation")
            self.assertEqual(report["team"]["team_name"], "ml-platform")
    ...
    def test_runtime_security_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_userns_and_kubelet_fine_grained_authz_for_release_workloads")
            ...
            self.assertIn("nodes/metrics", report["kubelet_rbac"]["allowed_subresources"])
        ...
        for expected in ["Runtime Security", "hostUsers: false", "fine-grained kubelet authorization", "nodes/metrics", "nodes/stats"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["ValidatingAdmissionPolicy", "nodes/pods", "allowPrivilegeEscalation: false", "seccompProfile", "RuntimeDefault"]:
            self.assertIn(expected, manifest)
    ...
    def test_control_plane_diagnostics_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(controller["name"] == "release-admission-controller" for controller in report["controllers"]))
        ...
        for expected in ["Control Plane Diagnostics", "/statusz", "/flagz", "PSI", "native histogram"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["ServiceMonitor", "PrometheusRule", "ReleaseControllerCacheStale", "KubeletPSIMemoryStallHigh", "NativeHistogramStorageBudgetHigh"]:
            self.assertIn(expected, manifest)
    ...
    def test_memory_qos_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(workload["qos_class"] == "Guaranteed" for workload in report["workloads"]))
        ...
        for expected in ["Memory QoS", "TieredReservation", "memory.min", "memory.low", "memory.high", "cgroup v2"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["KubeletConfiguration", "MemoryQoS", "PrometheusRule", "MemoryQoSThrottlingHigh", "MemoryQoSPSIPressureHigh"]:
            self.assertIn(expected, manifest)
    ...
    def test_hpa_scale_to_zero_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_hpa_scale_to_zero_for_elastic_noncritical_workers")
            ...
            self.assertFalse({"release-admission-controller", "online-inference-api"} & {workload["name"] for workload in report["scale_to_zero_workloads"]})
        ...
        for expected in ["HPA Scale To Zero", "HPAScaleToZero", "minReplicas: 0", "External", "cold-start"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["HorizontalPodAutoscaler", "autoscaling/v2", "type: Object", "MLOpsScaleToZeroWakeupFailed", "MLOpsScaleToZeroColdStartBudgetExceeded"]:
            self.assertIn(expected, manifest)
    ...
    def test_suspended_job_resource_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_suspended_job_resource_mutation_for_queued_ml_jobs")
        ...
        for expected in ["Suspended Job Resource Mutation", "MutablePodResourcesForSuspendedJobs", "spec.suspend", "CPU", "GPU"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["ValidatingAdmissionPolicy", "suspend: true", "MLOpsSuspendedJobResizeStale", "MLOpsSuspendedJobUnsuspendWithoutQuotaFit"]:
            self.assertIn(expected, manifest)
    ...
    def test_constrained_impersonation_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_constrained_impersonation_for_release_debugging")
        ...
        for expected in ["Constrained Impersonation", "ConstrainedImpersonation", "impersonate:serviceaccount", "impersonate-on:serviceaccount:get", "impersonate-on:serviceaccount:patch", "authentication.k8s.io", "audit"]:
            self.assertIn(expected, docs + manifest)
        for expected in ["RoleBinding", "rollback-controller-impersonate-actions", "release-status-writer", "ConfigMap", "MLOpsLegacyImpersonateVerbDetected", "MLOpsConstrainedImpersonationAuditMissing", "authenticationMetadata.impersonationConstraint"]:
            self.assertIn(expected, manifest)
    ...
    def test_event_driven_assets_plan_and_docs_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_airflow3_event_driven_assets")
            self.assertEqual(report["asset_expression"], "(RAW_EVENTS & CANDIDATE_MODEL) | ROLLBACK_REQUEST")
    ...
    def test_cohort_fair_sharing_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kueue_cohort_fair_sharing")
    ...
    def test_flavor_fungibility_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kueue_flavor_fungibility")
            self.assertTrue(all(policy["when_can_borrow"] == "TryNextFlavor" for policy in report["flavor_policies"]))
            self.assertTrue(any(policy["name"] == "gpu-canary-analysis" for policy in report["flavor_policies"]))
    ...
    def test_pending_workload_visibility_plan_and_kubernetes_assets_exist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["passed"])
            self.assertEqual(report["recommended_action"], "enable_kueue_pending_workload_visibility")
            self.assertEqual(report["feature"]["api_group"], "visibility.kueue.x-k8s.io/v1beta2")
            self.assertTrue(any(item["position"] == 1 for item in report["pending_workloads"]))
            self.assertTrue((root / "reports" / "pending_workload_visibility_plan.json").exists())
        for expected in ["visibility.kueue.x-k8s.io", "pendingworkloads", "kueue_admission_wait_time_seconds", "kueue_cluster_queue_resource_pending"]:
            self.assertIn(expected, manifest)
        for expected in ["VisibilityOnDemand", "ClusterQueue", "LocalQueue", "Prometheus"]:
            self.assertIn(expected, docs)
