...
def build_orchestration_scorecard(
    ...
    *,
    ...
    checks = [
        ...
        ("kserve_canary_server_side_apply", _present(content, "kserve_canary_readiness_plan.json", "--dry-run=server", "--field-manager=mlops-release-controller") and _present(content, "canaryTrafficPercent", "AnalysisTemplate"), "KServe canary promotion uses Server-Side Apply dry-run, field ownership, traffic split, and Argo analysis gates"),
        ...
        ("opencost_finops", _present(content, "cost_observability_report.json", "OpenCost", "node_gpu_hourly_cost") and _present(content, "MLOpsMonthlyCostBudgetExceeded"), "OpenCost and Prometheus budget alerts attribute serving, training, and canary-analysis spend"),
        ...
        ("dra_resource_health_status", _present(content, "resource_health_status_plan.json", "ResourceHealthStatus", "allocatedResourcesStatus") and _present(content, "ResourceClaim status.devices", "DeviceTaintRule"), "Kubernetes v1.36 DRA ResourceHealthStatus exposes unhealthy accelerator claims before canary promotion"),
        ...
        ("dra_admin_access_diagnostics", _present(content, "admin_access_diagnostics_plan.json", "DRAAdminAccess", "adminAccess: true") and _present(content, "resource.kubernetes.io/admin-access", "mlops-dra-admin"), "Kubernetes v1.36 DRA AdminAccess diagnostics inspect in-use devices through isolated privileged ResourceClaims"),
        ...
        ("airflow_stateful_orchestration", all(token in content for token in ["airflow_stateful_orchestration_plan.json", "task_state_store", "asset_state_store", "RollupMapper", "FanOutMapper", "PartitionedAtRuntime", "apache-airflow==3.3.0", "airflow-sdk-contract"]), "Airflow 3.3 release DAGs use scoped state stores, bounded partition mapping, exception-aware retries, and a real SDK parse gate"),
        ...
        ("scheduling_gate_controller", _present(content, "scheduling_gate_controller_plan.json", "mlops-scheduling-gate-controller") and _present(content, "mlops_scheduling_gate_age_seconds", "fail_closed_keep_pod_scheduling_gated"), "A fail-closed controller removes Pod Scheduling Readiness gates only after release evidence, Kueue, KServe, model-cache, and DRA signals are ready"),
        ...
        ("kueue_concurrent_admission", _present(content, "concurrent_admission_plan.json", "ConcurrentAdmission", "concurrentAdmissionPolicy") and _present(content, "TryPreferredFlavors", "lastAcceptableFlavorName", "concurrent-admission-parent"), "Kueue Concurrent Admission models Parent and Variant Workloads, preferred-flavor migration, flavor-scoped checks, and bounded fallback movement"),
    ...
    report = {
        ...
        "research_basis": [
            ...
            "KServe canary rollouts with canaryTrafficPercent, Server-Side Apply managed fields, server dry-run, and Argo AnalysisTemplates",
            ...
            "OpenCost exporter metrics for Kubernetes namespace, label, GPU, and workload cost allocation",
            ...
            "Kubernetes v1.36 DRA ResourceHealthStatus, ResourceClaim status.devices, and DeviceTaintRule quarantine for accelerator failure recovery",
            "Kubernetes DRA prioritized alternatives, partitionable devices, consumable capacity, and device binding conditions for accelerator efficiency",
            "Kubernetes v1.36 DRA AdminAccess for namespace-scoped maintenance and troubleshooting access to devices already in use",
            ...
            "Controller-driven Pod Scheduling Readiness gate removal with stale-gate incidents, DRA allocation checks, and fail-closed behavior",
            ...
            "Kueue Concurrent Admission for Parent and Variant Workloads, TryPreferredFlavors migration, lastAcceptableFlavorName boundaries, and parallel flavor-scoped checks",
